...
use super::client::NutClient;
use super::constants;

/// Nutaraline provider — errors do not lock or deactivate keys. Each request
/// tries every key at most once, then returns the last upstream error.
pub struct NutProvider {
...
impl NutProvider {
    ...
    fn build_body(&self, request: &ChatCompletionRequest, stream: bool) -> serde_json::Value {
        let model_name = request.model.strip_prefix("nut/").unwrap_or(&request.model);
...
impl Provider for NutProvider {
    ...
    async fn chat_completion(
        ...
        for _ in 0..total {
            ...
            let cred = match NutCredential::parse(&key.key_value) {
                Ok(c) => c,
                Err(e) => {
        ...
        Err(GatewayError::NoAvailableKeys(
            "No Nutaraline keys configured".into(),
        ))
    }
