...
impl IxProvider {
    pub fn new_with_keys(keys: Vec<ApiKey>, db: Arc<SqlitePool>) -> Self {
        ...
        Self { metadata, keys: KeyManager::new_with_pool(keys, constants::PROVIDER_ID, Some((*db).clone())), client: IxClient::new() }
    }
    ...
    fn exhausted(&self) -> GatewayError {
        GatewayError::NoAvailableKeys(format!(
            "All InferX keys exhausted ({} key(s) tried, none locked — errors are non-locking for this provider)",
