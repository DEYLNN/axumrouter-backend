...
impl Provider for KlProvider {
    ...
    async fn chat_completion(&self, request: ChatCompletionRequest) -> Result<ChatResult, GatewayError> {
        ...
        for attempt in 0..total.max(1) {
            ...
            let cred = match KcOAuthCredential::parse(&key.key_value) {
                Ok(c) => c,
                Err(e) => {
                    self.keys.lock_key(&key.id, 401, e.clone());
                    failed.push(FailedKeyAttempt { key_id: key_id.clone(), error: GatewayError::ProviderError(e) });
                    continue;
                }
            };
            ...
            match self.client.send_collect(body, &cred).await {
                ...
                Err(e) => {
                    let classified = lock_key_on_error(&self.keys, &key_id, &e);
                    if classified.retryable {
                        tracing::warn!("KiloCode key '{}' failed attempt {}/{}, kind={:?}", key_id, attempt + 1, total, classified.kind);
    ...
    async fn authenticate(&self) -> Result<(), GatewayError> {
        ...
        KcOAuthCredential::parse(&key.key_value)
            .map(|_| ())
