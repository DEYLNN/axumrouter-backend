...
pub async fn start() -> Json<serde_json::Value> {
    match crate::providers::oauth::codebuddy_intl::oauth::start().await {
        Ok(data) => Json(data), Err(e) => Json(serde_json::json!({"error": e})),
    }
}

pub async fn poll(State(state): State<Arc<AppState>>, Json(body): Json<serde_json::Value>) -> Json<serde_json::Value> {
    let code = body.get("device_code").and_then(|v| v.as_str()).unwrap_or_default();
    match crate::providers::oauth::codebuddy_intl::oauth::poll(code).await {
        Ok(data) => {
            if data.get("ok").and_then(|v| v.as_bool()).unwrap_or(false) {
                if let Err(e) = crate::providers::oauth::codebuddy_intl::oauth::save_token(&state, &data).await { return Json(serde_json::json!({"ok": false, "error": e})); }
            }
            Json(data)
        }
...
pub async fn refresh(State(state): State<Arc<AppState>>, Path(key_id): Path<String>) -> Json<serde_json::Value> {
    let row = sqlx::query_as::<_, (String,)>("SELECT key_value FROM api_keys WHERE id = ? AND provider_id = 'cbai' AND key_type = 'oauth' AND is_active = 1")
        .bind(&key_id).fetch_optional(&state.db).await.unwrap_or(None);
    let Some((raw,)) = row else { return Json(serde_json::json!({"ok": false, "error": "CodeBuddy key not found"})); };
    ...
    match crate::providers::oauth::codebuddy_intl::oauth::refresh(rt).await {
        Ok(data) => {
