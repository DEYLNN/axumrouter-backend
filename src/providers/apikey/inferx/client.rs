...
impl IxClient {
    ...
    fn headers(&self, builder: reqwest::RequestBuilder, cred: &IxCredential) -> reqwest::RequestBuilder {
        builder
    ...
    pub async fn send_collect(&self, body: Value, cred: &IxCredential) -> Result<ChatCompletionResponse, GatewayError> {
        let url = format!("{}/v1/chat/completions", constants::BASE_URL);
        let response = self.headers(self.http.post(&url), cred)
            .json(&body)
            .send()
            .await
            .map_err(|e| GatewayError::ProviderError(format!("Ix HTTP: {}", e)))?;
        ...
        let json: Value = response.json().await
            .map_err(|e| GatewayError::ProviderError(format!("Ix parse: {}", e)))?;
        ...
        // InferX models always think — drop reasoning_content, strip think-block tags.
    ...
    pub async fn send_stream(&self, body: Value, cred: &IxCredential) -> Result<BoxStream<'static, Result<ChatCompletionChunk, GatewayError>>, GatewayError> {
        let url = format!("{}/v1/chat/completions", constants::BASE_URL);
        let response = self.headers(self.http.post(&url), cred)
            .json(&body)
            .send()
    ...
    fn parse_chunk(v: &Value, model: &str, usage: &mut Option<Usage>, content_buffer: &mut String) -> Option<ChatCompletionChunk> {
        ...
        Some(ChatCompletionChunk {
            id: format!("chatcmpl-inferx-{}", chrono::Utc::now().timestamp()),
