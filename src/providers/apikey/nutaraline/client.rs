...
impl NutClient {
    ...
    pub async fn send_collect(
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| GatewayError::ProviderError(format!("Nut HTTP: {}", e)))?;
        ...
        let json: Value = response
            .json()
            .await
            .map_err(|e| GatewayError::ProviderError(format!("Nut parse: {}", e)))?;
        ...
        // Nutaraline models always think — drop reasoning_content, strip think-block tags.
        ...
        let parsed = async_stream::try_stream! {
            ...
            loop {
                ...
                let next = tokio::time::timeout(wait, upstream.next()).await
                    .map_err(|_| GatewayError::ProviderError(format!("Nut stream timeout: {}s", wait.as_secs())))?;
                let Some(maybe_bytes) = next else { break };
                let bytes = maybe_bytes.map_err(|e| GatewayError::ProviderError(format!("Nut stream read: {}", e)))?;
            ...
            if let Some(u) = collected_usage.take() {
                yield ChatCompletionChunk {
                    id: format!("chatcmpl-nut-{}", chrono::Utc::now().timestamp()),
