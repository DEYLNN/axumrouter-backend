...
impl FsnClient {
    ...
    pub async fn send_collect(&self, body: Value, cred: &FsnCredential) -> Result<ChatCompletionResponse, GatewayError> {
        ...
        Ok(ChatCompletionResponse {
            id: json.get("id").and_then(|v| v.as_str()).unwrap_or("fsn-unknown").to_string(),
        ...
        let parsed = async_stream::try_stream! {
            ...
            loop {
                ...
                let next = tokio::time::timeout(wait, upstream.next()).await
                    .map_err(|_| GatewayError::ProviderError(format!("Fsn stream timeout: {}s", wait.as_secs())))?;
                let Some(maybe_bytes) = next else { break };
                let bytes = maybe_bytes.map_err(|e| GatewayError::ProviderError(format!("Fsn stream read: {}", e)))?;
