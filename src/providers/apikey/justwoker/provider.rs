...
use super::auth::JwkCredential;
use super::client::JwkClient;
use super::constants;

/// JustWoker provider — errors do not lock or deactivate keys. Each request
...
impl JwkProvider {
    ...
    fn build_body(&self, request: &ChatCompletionRequest, stream: bool) -> serde_json::Value {
        let model_name = request.model.strip_prefix("jwk/").unwrap_or(&request.model);
...
impl Provider for JwkProvider {
    ...
    async fn chat_completion(
        ...
        for _ in 0..total {
            ...
            let cred = match JwkCredential::parse(&key.key_value) {
                Ok(c) => c,
                Err(e) => {
        ...
        Err(GatewayError::NoAvailableKeys(
            "No JustWoker keys configured".into(),
        ))
    }
