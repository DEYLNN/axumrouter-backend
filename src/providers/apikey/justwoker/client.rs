...
impl JwkClient {
    ...
    pub async fn send_collect(
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| GatewayError::ProviderError(format!("Jwk HTTP: {}", e)))?;
        ...
        let json: Value = response
            .json()
            .await
            .map_err(|e| GatewayError::ProviderError(format!("Jwk parse: {}", e)))?;
        ...
        // JustWoker models always think — drop reasoning_content, strip think-block tags.
        ...
        let parsed = async_stream::try_stream! {
            ...
            if let Some(u) = collected_usage.take() {
                yield ChatCompletionChunk {
                    id: format!("chatcmpl-jwk-{}", chrono::Utc::now().timestamp()),
