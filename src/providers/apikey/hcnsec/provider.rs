...
use super::auth::HcnCredential;
use super::client::HcnClient;
use super::constants;

/// Hcnsec provider — errors do not lock or deactivate keys. Each request
/// tries every key at most once, then returns the last upstream error.
pub struct HcnProvider {
...
impl HcnProvider {
    ...
    fn build_body(&self, request: &ChatCompletionRequest, stream: bool) -> serde_json::Value {
        let model_name = request.model.strip_prefix("hcn/").unwrap_or(&request.model);
...
impl Provider for HcnProvider {
    ...
    async fn chat_completion(
        ...
        for _ in 0..total {
            ...
            let cred = match HcnCredential::parse(&key.key_value) {
                Ok(c) => c,
                Err(e) => {
        ...
        Err(GatewayError::NoAvailableKeys(
            "No Hcnsec keys configured".into(),
        ))
    }
