...
impl HcnClient {
    ...
    pub async fn send_collect(
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| GatewayError::ProviderError(format!("Hcn HTTP: {}", e)))?;
        ...
        let json: Value = response
            .json()
            .await
            .map_err(|e| GatewayError::ProviderError(format!("Hcn parse: {}", e)))?;
        ...
        // Hcnsec models always think — drop reasoning_content, strip think-block tags.
        ...
        let parsed = async_stream::try_stream! {
            ...
            if let Some(u) = collected_usage.take() {
                yield ChatCompletionChunk {
                    id: format!("chatcmpl-hcn-{}", chrono::Utc::now().timestamp()),
