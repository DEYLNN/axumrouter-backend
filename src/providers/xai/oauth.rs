...
pub async fn save_token(state: &Arc<crate::state::AppState>, token: &serde_json::Value) -> Result<(), String> {
    ...
    let label = extract_email(token).unwrap_or_else(|| format!("xai-{}", &kid[4..12]));

    sqlx::query("INSERT INTO api_keys (id, provider_id, key_value, label, is_active, key_type, created_at, updated_at) VALUES (?, 'xai', ?, ?, 1, 'oauth', ?, ?)")
