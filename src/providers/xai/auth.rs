...
impl XaiOAuthCredential {
    pub fn parse(raw: &str) -> Result<Self, GatewayError> {
        let cred: Self = serde_json::from_str(raw)
            .map_err(|e| GatewayError::ProviderError(format!("xAI key must be OAuth JSON: {}", e)))?;
        if cred.disabled {
            return Err(GatewayError::ProviderError("xAI credential disabled".into()));
        }
        if cred.access_token.trim().is_empty() && cred.refresh_token.trim().is_empty() {
            return Err(GatewayError::ProviderError("xAI OAuth JSON missing access_token/refresh_token".into()));
        }
        Ok(cred)
    }
