...
#[derive(Debug, Clone)]
pub struct CbaiOAuthCredential {
    pub access_token: String,
    pub email: Option<String>,
    pub org_id: Option<String>,
}

impl CbaiOAuthCredential {
    pub fn parse(kv: &str) -> Result<Self, String> {
        let val: serde_json::Value =
            serde_json::from_str(kv).map_err(|e| format!("CodeBuddy: invalid key_value JSON: {e}"))?;
        let access_token = val["access_token"]
            .as_str()
            .or_else(|| val["accessToken"].as_str())
            .ok_or_else(|| "CodeBuddy: missing access_token".to_string())?
            .to_string();
